...
function App() {
  ...
  const fetchData = async () => {
    try {
      ...
      setError("System Offline! Python Terminal restart karke dekhein.");
    } finally {
      setLoading(false);
    }
  };
  ...
  // ── HOME VIEW (Main Predictor Dashboard) ──
  return (
    <div className="app-root">
      <div className="canvas-bg">
        <Hero3D prediction={data.prediction} />
      </div>
      <div className="grain" />
        ...
        {isMenuOpen && (
          <>
            <motion.div 
              ...
              style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(5px)', zIndex: 99 }}
            />
            
            <motion.div 
              initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }}
              transition={{ type: 'spring', damping: 25, stiffness: 200 }}
              style={{ position: 'fixed', top: 0, right: 0, bottom: 0, width: '320px', background: 'rgba(15, 23, 42, 0.95)', borderLeft: '1px solid rgba(255,255,255,0.1)', zIndex: 100, padding: '2rem', display: 'flex', flexDirection: 'column' }}
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2.5rem' }}>
                ...
                <button onClick={() => setIsMenuOpen(false)} style={{ background: 'transparent', border: 'none', color: '#94a3b8', cursor: 'pointer' }}>
                  <X size={24} />
                </button>
              </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                <button 
                  onClick={() => { setIsMenuOpen(false); setActiveView('oi'); }} 
                  ...
                  onMouseOver={(e) => { e.currentTarget.style.borderColor = '#22d3ee'; e.currentTarget.style.background = 'rgba(34, 211, 238, 0.1)'}}
                  onMouseOut={(e) => { e.currentTarget.style.borderColor = 'transparent'; e.currentTarget.style.background = 'rgba(255,255,255,0.05)'}}
                >
